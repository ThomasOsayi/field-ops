...
export default function IntegrationsPage() {
  ...
  // Check connection on load and after redirect
  useEffect(() => {
    const check = async () => {
    ...
    // Check URL params for redirect result
  ...
  const handleSyncNow = async () => {
    // TODO: trigger a full re-sync of all active jobs
          ...
          {checking ? (
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '16px', marginBottom: '32px' }}>
                <div
                  onClick={() => connected ? setConfigOpen(true) : handleConnect()}
                ...
                >
                  ...
                  <div style={{ fontSize: '13px', color: 'var(--text-muted)', lineHeight: 1.5, marginBottom: '16px' }}>
                    {connected
                      ? 'Jobs automatically sync to your Outlook calendar. Click to configure settings.'
                      : 'Connect your Microsoft account to sync jobs to Outlook Calendar.'}
                  </div>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '620px', background: 'var(--bg-sidebar)', borderLeft: '1px solid var(--border)', zIndex: 300, display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ padding: '28px', borderBottom: '1px solid var(--border)', background: 'linear-gradient(135deg, rgba(76,158,235,0.08), rgba(123,97,255,0.05))', display: 'flex', alignItems: 'center', gap: '20px' }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', fontWeight: 600, color: 'var(--success)' }}>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ width: '14px', height: '14px' }}><polyline points="20 6 9 17 4 12"/></svg>
                    Connected · {email || 'thomas@osayi.com'}
                  </div>
                </div>
              </div>
              ...
              <div style={{ padding: '24px 28px' }}>
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
                    {renderInfoRow('Account', email || 'thomas@osayi.com', { mono: true, accent: true })}
            ...
            <div style={{ padding: '18px 28px', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
              <button onClick={handleDisconnect} disabled={disconnecting}
                style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', padding: '8px 14px', borderRadius: 'var(--radius-sm)', fontFamily: 'var(--font)', fontSize: '12px', fontWeight: 700, cursor: 'pointer', background: 'var(--danger-muted)', color: 'var(--danger)', border: '1px solid rgba(248,113,113,0.2)', transition: 'all 0.15s', opacity: disconnecting ? 0.6 : 1 }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button onClick={handleSyncNow} style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', padding: '9px 16px', borderRadius: 'var(--radius-sm)', fontFamily: 'var(--font)', fontSize: '13px', fontWeight: 700, cursor: 'pointer', background: 'linear-gradient(135deg, #4C9EEB, #7B61FF)', color: '#fff', border: 'none', boxShadow: '0 4px 16px rgba(76,158,235,0.3)', transition: 'all 0.15s' }}>
