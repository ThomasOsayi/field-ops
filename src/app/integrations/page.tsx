...
export default function IntegrationsPage() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Get recent sync-related notifications for history
        const notifsQ = query(
        ...
        notifsSnap.docs.forEach(doc => {
          ...
          // Include outlook and job-related notifications
          if (category === 'outlook' || category === 'jobs' || category === 'calendar') {
            const createdAt = d.createdAt?.toDate ? d.createdAt.toDate() : new Date(d.createdAt);
            if (!latestSyncTime || createdAt > latestSyncTime) {
              latestSyncTime = createdAt;
            }
            history.push({
              id: doc.id,
              message: d.title || d.body || 'Sync event',
              detail: formatSyncDate(createdAt) + (d.body ? ` · ${d.body.slice(0, 50)}` : ''),
              status: d.type === 'error' ? 'warn' : 'ok',
            });
          }
        });
        ...
        if (latestSyncTime) {
          setLastSyncLabel(timeAgo(latestSyncTime));
        }
      } catch (err) {
  ...
  const handleSyncNow = async () => {
    alert('Manual sync triggered — all active jobs will be synced to Outlook.');
  };
  ...
  const renderToggle = (label: string, sub: string, value: boolean, onChange: () => void) => (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
      ...
      <div onClick={onChange} style={{ width: '40px', height: '22px', borderRadius: '11px', background: value ? 'var(--accent)' : 'var(--border)', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
                ...
                <div onClick={() => connected ? setConfigOpen(true) : handleConnect()}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '16px' }}>
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '4px 12px', borderRadius: '20px', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.03em', background: connected ? 'var(--success-muted)' : 'var(--bg-elevated)', color: connected ? 'var(--success)' : 'var(--text-muted)' }}>
                      <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: connected ? 'var(--success)' : 'var(--text-muted)' }} />
                  ...
                  <div style={{ fontSize: '13px', color: 'var(--text-muted)', lineHeight: 1.5, marginBottom: '16px' }}>
                    {connected ? 'Jobs automatically sync to your Outlook calendar. Click to configure settings.' : 'Connect your Microsoft account to sync jobs to Outlook Calendar.'}
                  </div>
                  {connected ? (
                    ...
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', padding: '9px 16px', borderRadius: 'var(--radius-sm)', fontSize: '13px', fontWeight: 700, background: 'linear-gradient(135deg, #4C9EEB, #7B61FF)', color: '#fff', boxShadow: '0 4px 16px rgba(76,158,235,0.3)' }}>
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ width: '16px', height: '16px' }}><rect x="2" y="4" width="20" height="16" rx="2"/><polyline points="22,7 12,13 2,7"/></svg>
                      Connect Outlook
                    </div>
                  )}
                </div>
        ...
        {configOpen && <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '620px', background: 'var(--bg-sidebar)', borderLeft: '1px solid var(--border)', zIndex: 300, display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 28px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              ...
              <button onClick={() => setConfigOpen(false)} style={{ width: '36px', height: '36px', borderRadius: 'var(--radius-sm)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--text-muted)', background: 'transparent', border: '1px solid var(--border)' }}>
              ...
              <div style={{ padding: '24px 28px' }}>
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
                    ...
                    <div style={{ borderBottom: 'none' }}>{renderToggle('Include job notes', 'Add scope and notes to the calendar event body', includeNotes, () => setIncludeNotes(!includeNotes))}</div>
                  </div>
                </div>
            ...
            <div style={{ padding: '18px 28px', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
              <button onClick={handleDisconnect} disabled={disconnecting}
                style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', padding: '8px 14px', borderRadius: 'var(--radius-sm)', fontFamily: 'var(--font)', fontSize: '12px', fontWeight: 700, cursor: 'pointer', background: 'var(--danger-muted)', color: 'var(--danger)', border: '1px solid rgba(248,113,113,0.2)', opacity: disconnecting ? 0.6 : 1 }}>
